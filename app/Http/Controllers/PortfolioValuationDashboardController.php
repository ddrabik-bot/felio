...
final class PortfolioValuationDashboardController extends Controller
{
    ...
    {
        return ['instrument' => $row->instrument, 'quantity' => $row->quantity, 'availability' => $row->availability->value, 'sourcePrice' => ['amount' => $row->sourcePrice->pricePerUnit, 'currency' => $row->sourcePrice->currency, 'availability' => $row->sourcePrice->availability->value, 'diagnostic' => $row->sourcePrice->diagnostic], 'fx' => $row->fxRate === null ? ['status' => $row->sourcePrice->currency === 'PLN' ? 'not_required' : 'unavailable', 'amount' => null, 'currency' => $row->sourcePrice->currency === 'PLN' ? null : $row->sourcePrice->currency, 'diagnostic' => $row->sourcePrice->currency === 'PLN' ? null : 'fx_rate_missing'] : ['status' => $row->fxRate->availability->value, 'amount' => $row->fxRate->plnPerUnit, 'currency' => $row->fxRate->currency, 'diagnostic' => $row->fxRate->reason], 'plnGrosze' => $row->plnGrosze, 'diagnostics' => $row->diagnostics];
    }
}
