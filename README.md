...
Public session authentication is available through Inertia pages at `/register`, `/login`, and `/forgot-password`. Registration creates a password-hashed user, starts a regenerated session, and redirects to `/portfolio/onboarding`, where the authenticated user creates their active XTB portfolio before visiting the import or valuation pages. The portfolio is scoped to the authenticated user; workbook account references are source metadata, so imports are persisted into the currently selected active portfolio regardless of the XTB account that produced the workbook. Login and logout use Laravel's `web` session guard. Password reset links use Laravel's reset-token broker and are delivered through the configured mailer (the test environment uses Laravel's array mailer). Authentication does not persist uploaded workbooks or alter the existing portfolio-import workflow.
